...
btv.bh.HeapifyUpAlg.prototype.rec = function(node) {
    ...
    var difference =  this.node.getValue()- parent.getValue() ;
...
btv.bh.HeapifyDownAlg.prototype.rec = function(node) {
    ...
    var difference = node.getValue() - lesser.getValue();
    
    if(lesser === left) { // compare with left
        this.visualiser.animateShowComparSign(lesser, -difference, node);
    } else { // compare with right
        this.visualiser.animateShowComparSign(node, difference, lesser); 
    }
    
    if(difference > 0) {
        
        this.tree.swapNodes(node, lesser);
        this.visualiser.animateSwapNodeElems(node, lesser);
...
btv.bh.BuildHeapAlg.prototype.redo = function() {
    ...
    for(i = Math.floor(array.length/2) - 1; i >= 0; i--) {
        ...
        var heapifyAlg = new btv.bh.HeapifyUpAlg(this.tree, this.visualiser, this.tree.getNode(i), true);
        heapifyAlg.redo();
    }
